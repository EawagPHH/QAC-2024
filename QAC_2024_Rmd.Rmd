...
```{r}
library(car)
lmTlog = lm(dat$logll ~ log(dat$CT,10) +dat$size10 + dat$carbon + as.numeric(dat$Temp) +dat$Surface  + dat$Gsize +dat$liquid +dat$measurement +dat$`ssRNA(-)`+ dat$ssDNA +dat$`ssRNA(+)` + dat$dsRNA +  dat$BZT + dat$DDAB + dat$DDAC + dat$MBAT + dat$others + dat$QACmix )
summary(lmTlog)

#without genomesize and liquid
lmTlog = lm(dat$logll ~ log(dat$CT,10) +dat$size10 + dat$carbon + as.numeric(dat$Temp) +dat$Surface   +dat$measurement +dat$`ssRNA(-)` + dat$ssDNA +dat$`ssRNA(+)` + dat$dsRNA +  dat$BZT + dat$DDAB + dat$DDAC + dat$MBAT + dat$others + dat$QACmix )
summary(lmTlog)

#without genomesize, carbon load, liquid
lmTlog = lm(dat$logll ~ log(dat$CT,10) +dat$size10 + as.numeric(dat$Temp) +dat$Surface  +dat$measurement+dat$`ssRNA(-)` + dat$ssDNA +dat$`ssRNA(+)` + dat$dsDNA +  dat$BZT + dat$DDAB + dat$DDAC + dat$MBAT + dat$others + dat$QACmix )
summary(lmTlog)



lmTlog = lm(dat$log ~ log(dat$CT,10) +dat$size10 + dat$carbon + as.numeric(dat$Temp) +dat$Surface  + dat$Gsize + dat$"ssRNA(-)"+dat$"ssRNA(+)" +dat$dsDNA +dat$dsRNA +dat$ssDNA)
summary(lmTlog)
library(car)
vif(lmTlog)
alias(lmTlog) 

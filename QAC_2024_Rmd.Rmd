...
subset_LR <-  subset(dat, (!is.na(dat$log)) &
                     dat$`time (min)` <= 30 
                      )
...
#test correlation for non numerical variables single handed
 ...
 cor(dat$`ssRNA(-)`,dat$`ssRNA(-)`)
 cor(dat$`ssRNA(-)`,dat$carbon)
 cor(dat$`ssRNA(-)`,dat$Surface)

 #exclude Gsize or size due to correlation 


 
 # Fit the initial model including all columns
lmTlog = lm(logll ~ log(CT, 10) + size10 + carbon + as.numeric(Temp) + Surface +  measurement + `ssRNA(-)` + ssDNA + `ssRNA(+)` + dsRNA + BZT + DDAB + DDAC  + others + QACmix, data = dat)
